...
# filter for gubernatorial races and basic calculations of winning margins
governor_filter <- function(df){
  governor_races <- df %>%
    filter(str_detect(CONTEST_FULL_NAME, "Governor")) %>%
    select(PRECINCT_NAME, CANDIDATE_FULL_NAME, candidate_party_id, CONTEST_FULL_NAME, TOTAL, PRECINCT_ID, CONTEST_TOTAL) %>%
    mutate(TOTAL_PCT = ifelse(TOTAL != 0, TOTAL/CONTEST_TOTAL, 0)) %>%
    group_by(PRECINCT_ID) %>%
    arrange(desc(TOTAL)) %>%
...
# pivot to wider table for each precinct
tidy_governor_election_data <- function(df, csv, location){
  tidy_df <- df %>%
    filter(candidate_party_id %in% c(1, 2)) %>%
    pivot_wider(id_cols = c(PRECINCT_NAME, CANDIDATE_FULL_NAME, CONTEST_FULL_NAME, CONTEST_TOTAL,
                            WINNER, WINNING_PARTY, MARGIN_PCT, MARGIN
...
maricopa_2018_governor <- maricopa_2018_results %>%
  governor_filter() %>%
  tidy_governor_election_data(csv = T, location = "./CongressRaceResults/maricopa_2018_governor.csv")

maricopa_2018_voter_registration <- maricopa_2018_results %>%
  filter(CONTEST_FULL_NAME == "Registration & Turnout") %>%
  select(PRECINCT_NAME, PRECINCT_ID, CONTEST_TOTAL) %>%
  unique()
write_csv(maricopa_2018_voter_registration, "./CongressRaceResults/maricopa_2018_voter_registration.csv")
